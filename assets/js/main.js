...
/* ===== V18 tasteful motion system ===== */
(() => {
  const reduceMotion = window.matchMedia &&
    window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // Fade the page in after styles and markup are ready.
  ...
  // Assign reveal behavior automatically so HTML stays clean.
  const revealGroups = [
    ['section:not(.page-hero) .eyebrow, section:not(.page-hero) .section-title, section:not(.page-hero) .lead', 'reveal'],
    ['.image-panel, .youtube-card, .offering-card, .cta-panel, .hero-seal-card', 'reveal reveal-scale'],
    ['.timeline-item, .person-card, .ministry-card, .link-card, .service-card, .feature', 'reveal stagger-item'],
    ['.quote-card, .archive-note, .contact-row', 'reveal']
  ];
  ...
  // Give repeated groups a gentle stagger.
  [
    '.schedule-grid',
    '.grid-3',
    '.grid-2',
    ...
    document.querySelectorAll(groupSelector).forEach(group => {
      [...group.children].forEach((child, index) => {
        ...
        child.style.setProperty('--stagger-index', Math.min(index, 8));
      });
    });
  });

  // Alternate image/copy direction on split layouts.
  document.querySelectorAll('.split').forEach(split => {
    const kids = [...split.children];
    if (kids[0]) kids[0].classList.add('reveal', 'reveal-left');
    if (kids[1]) kids[1].classList.add('reveal', 'reveal-right');
  });
  ...
  // Give the Bible passage a small visual refresh on language switch.
  const passageCard = document.querySelector('.daily-passage-card');
  const languageButtons = document.querySelectorAll('.language-btn');
  languageButtons.forEach(button => {
    button.addEventListener('click', () => {
      if (!passageCard) return;
      passageCard.classList.remove('passage-switching');
      void passageCard.offsetWidth;
      passageCard.classList.add('passage-switching');
      window.setTimeout(() => passageCard.classList.remove('passage-switching'), 480);
    });
  });
})();
/* ===== end V18 tasteful motion system ===== */
